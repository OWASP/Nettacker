...
def __wizard(
    ...
    targets = __input(
        messages(language, "enter_default").format(
            messages(language, "all_targets"), default_config["targets"]
        ),
        default_config["targets"],
    )
    thread_number = __input(
        messages(language, "enter_default").format(
            messages(language, "all_thread_numbers"), default_config["thread_number"]
        ),
    ...
    log_in_file = __input(
        messages(language, "enter_default").format(
            messages(language, "out_file"), default_config["log_in_file"]
        ),
        default_config["log_in_file"],
    )
    scan_method = __input(
        messages(language, "enter_choices_default").format(
            messages(language, "all_scan_methods"),
            ", ".join(module_name),
    ...
    users = __input(
        messages(language, "enter_default").format(
            messages(language, "all_usernames"), default_config["users"]
        ),
        default_config["users"],
    )
    passwds = __input(
        messages(language, "enter_default").format(
            messages(language, "all_passwords"), default_config["passwds"]
        ),
        default_config["passwds"],
    )
    timeout_sec = __input(
        messages(language, "enter_default").format(
            messages(language, "timeout_seconds"), default_config["timeout_sec"]
        ),
    ...
    ports = __input(
        messages(language, "enter_default").format(
            messages(language, "all_ports"), default_config["ports"]
        ),
        default_config["ports"],
    )
    verbose_level = __input(
        messages(language, "enter_default").format(
            messages(language, "all_verbose_level"), default_config["verbose_level"]
        ),
        default_config["verbose_level"],
    )
    socks_proxy = __input(
        messages(language, "enter_default").format(
            messages(language, "all_socks_proxy"), default_config["socks_proxy"]
        ),
        default_config["socks_proxy"],
    )
    retries = __input(
        messages(language, "enter_default").format(
            messages(language, "retries_number"), default_config["retries"]
        ),
