...
class NettackIMAPLib:
    def imap_brute_force(host, ports, usernames, passwords, timeout):
        server = imaplib.IMAP4_SSL(host,port=ports,timeout=timeout)
...
class Engine:
    def run(
        ...
        action = getattr(NettackIMAPLib, backup_method, None)
