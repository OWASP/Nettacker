...
def multi_processor(options, scan_unique_id):
    ...
    for target_group in options.targets:
        for target in target_group:
            for module_name in options.selected_modules:
                redis_path = "." + redis.normalize_key_name(target)
                redis.write(
                    "nettacker_events",
                    redis_path,
                    {},
                ...
                redis_path += "." + redis.normalize_key_name(module_name)
                redis.delete(
                    "nettacker_events",
                    redis_path
                )
...
def start_scan_processes(options, scan_unique_id):
    """
    preparing for attacks and managing multiprocessing for host
    ...
    options.targets = expand_targets(
        options,
        scan_unique_id
    )
    if options.targets:
        exit_code = multi_processor(
            options,
            scan_unique_id
        )
