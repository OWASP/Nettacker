...
class NettackerModules:
    def __init__(self):
        ...
        self.libraries = [
            ...
            for module_protocol in os.listdir(
                nettacker_paths()["module_protocols_path"]
            )
    ...
    def start(self):
        ...
        for payload in self.module_content["payloads"]:
            protocol = getattr(
                __import__(
                    "core.module_protocols.core_{library}".format(
...
def load_all_graphs():
    ...
    for graph_library in glob(
        os.path.join(nettacker_paths()["home_path"] + "/lib/graph/*/engine.py")
    ):
...
def load_all_languages():
    ...
    for language in glob(
        os.path.join(nettacker_paths()["home_path"] + "/lib/messages/*.yaml")
    ):
...
def perform_scan(
    ...
    from core.alert import verbose_event_info, messages
