...
def load_all_graphs():
    ...
    for _lib in glob(os.path.dirname(inspect.getfile(lib)) + "/*/*/engine.py"):
        if (
            os.path.dirname(_lib).rsplit("\\" if is_windows() else "/")[-2] == "graph"
            and _lib + "_graph" not in graph_names
        ):
...
def __check_external_modules():
    ...
    for module in external_modules:
        try:
            ...
            __die_failure(
                "pip install -r requirements.txt ---> " + module + " not installed!"
            )
    ...
    if not os.path.exists(default_config["home_path"]):
        try:
            ...
            __die_failure(
                "cannot access the directory {0}".format(default_config["home_path"])
            )
    if not os.path.exists(default_config["tmp_path"]):
        try:
            ...
            __die_failure(
                "cannot access the directory {0}".format(default_config["results_path"])
            )
    ...
    if default_config["database_type"] == "sqlite":
        try:
            if os.path.isfile(
                default_config["home_path"] + "/" + default_config["database_name"]
            ):
                pass
            else:
