...
def build_graph(graph_name, events):
    ...
    try:
        start = getattr(
            __import__(
                "lib.graph.{0}.engine".format(graph_name.rsplit("_graph")[0]),
                fromlist=["start"],
            ),
            "start",
        )
    except Exception:
        die_failure(messages("graph_module_unavailable").format(graph_name))
...
def build_texttable(events):
    ...
    return (
        ...
        + b"\n\n"
        + messages("nettacker_version_details")
        .format(version_info()[0], version_info()[1], now())
        .encode("utf8")
        + b"\n"
    )
    ...
    if (len(report_path_filename) >= 5 and report_path_filename[-5:] == ".html") or (
        ...
        with open(report_path_filename, "a") as csvfile:
            ...
            for log in all_scan_logs:
                dict_data = {key: value for key, value in log.items() if key in keys}
