...
def remove_old_logs(host, type, scan_id, language):
    ...
    old_logs = session.query(HostsLog).filter(
        HostsLog.host == host, HostsLog.type == type, HostsLog.scan_id != scan_id
    )
...
def __get_result(language, id):
    ...
    try:
        try:
            ...
            return (
                jsonify(__structure(status="error", msg="cannot find the file!")),
                400,
            )
    except Exception as _:
...
def __last_host_logs(language, page):
    ...
    try:
        for host in (
            ...
            .group_by(HostsLog.host)
            .order_by(HostsLog.id.desc())[page : page + 11]
        ):
            for data in (
                session.query(HostsLog)
                .filter(HostsLog.host == host)
                .group_by(
                    HostsLog.type,
                    HostsLog.port,
                    HostsLog.username,
                    HostsLog.password,
                    HostsLog.description,
                )
...
def __logs_to_report_html(host, language):
    ...
    try:
        ...
        _table = _log_data.table_title.format(
            _graph,
            _log_data.css_1,
            "HOST",
            "USERNAME",
        ...
        _table += (
            _log_data.table_end
            + '<p class="footer">'
            + messages("en", "nettacker_report").format(
                compatible.__version__, compatible.__code_name__, now()
            )
            + "</p>"
        )
        return _table
...
def __search_logs(language, page, query):
    ...
    try:
        for host in (
            ...
            for data in (
                session.query(HostsLog)
                .filter(HostsLog.host == str(host.host))
                .group_by(
                    HostsLog.type,
                    HostsLog.port,
                    HostsLog.username,
