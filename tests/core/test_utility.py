from nettacker.core import utility
...
class TestUtility(TestCase):
    def test_sort_dictionary(self):
        ...
        sorted_dict_keys = tuple(utility.sort_dictionary(input_dict).keys())
    ...
    def test_select_maximum_cpu_core(self, cpu_count_mock):
        ...
        for num_cores, levels in cores_mapping.items():
            ...
            for level in ("low", "normal", "high", "maximum"):
                self.assertEqual(
                    utility.select_maximum_cpu_core(level),
                    levels[level],
                    f"It should be {utility.select_maximum_cpu_core(level)} of {num_cores} cores for '{level}' mode",
                )

            self.assertEqual(utility.select_maximum_cpu_core("invalid"), 1)
