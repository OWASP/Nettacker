...
class DSLMatcher:
        ...
        try:
            ...
            for splitter, combiner in [("||", any), (" or ", any), ("&&", all), (" and ", all)]:
                ...
                if splitter in ["||", "&&"]:
                    ...
                    check = re.search(
                        rf"\s+{re.escape(splitter.strip())}\s+", normalized, re.IGNORECASE
                    )
                ...
                if check:
                    parts = [
                        ...
                        for part in re.split(
                            rf"\s*(?:{re.escape(splitter)})\s*", normalized, flags=re.IGNORECASE
                        )
                        if part.strip()
                    ]
    ...
    def _evaluate_range(self, dsl_expression, target_version):
        ...
        try:
            ...
            ) and self._compare_version(target_version, end_version, "<=")
