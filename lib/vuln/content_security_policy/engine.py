...
def __content_policy(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if content_policy(target, port, timeout_sec, log_in_file, language, time_sleep,
        ...
        info(messages(language, "target_vulnerable").format(target, port,
                                                            'CSP makes it possible for server administrators to reduce or eliminate the vectors by which XSS can occur by specifying the domains that the browser should consider to be valid sources of executable scripts. '), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'content_security_policy_vuln',
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format(
                'Content Security Policy'), log-in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'content_security_policy_vuln',
