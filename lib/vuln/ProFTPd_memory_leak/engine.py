...
def __memory_leak(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if memory_leak(target, port, timeout_sec, log_in_file, language, time_sleep,
        ...
        info(messages(language, "target_vulnerable").format(target, port,
                                                            'Memory leak in ProFTPd 1.2.0rc2 allows remote attackers to cause a denial of service via a series of USER commands, and possibly SIZE commands if the server has been improperly installed.    CVE-2001-0136'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'ProFTPd_memory_leak_vuln',
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep, language,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        for port in ports:
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(trying, total_req, num, total, target, port, 'ProFTPd_memory_leak_vuln'))
            while 1:
                try:
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format(
                'ProFTPd_memory_leak  CVE-2001-0136'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'ProFTPd_memory_leak_vuln',
                               'DESCRIPTION': messages(language, "no_vulnerability_found").format('ProFTPd_memory_leak    CVE-2001-0136'), 'TIME': now(),
                               'CATEGORY': "scan", 'SCAN_ID': scan_id, 'SCAN_CMD': scan_cmd}, language, thread_tmp_filename)
        os.remove(thread_tmp_filename)

    else:
        warn(messages(language, "input_target_error").format(
            'ProFTPd_memory_leak_vuln', target))
