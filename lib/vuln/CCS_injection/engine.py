...
def extra_requirements_dict():
    return {"CCS_injection_vuln_ports": [21, 25, 110, 143, 443, 587, 990, 1080, 8080]}
        ...
        if iRecordLen + 5 < iSSLLen:
            ...
            while iNextOffset < len(strBuf):
                ...
                iTypeAndLen = struct.unpack(
                    ">I", strBuf[iNextOffset : iNextOffset + 4]
                )[0]
            ...
            while iNextOffset + 6 < len(strBuf):
                ...
                iRecordLen = struct.unpack(
                    ">H", strBuf[iNextOffset + 3 : iNextOffset + 5]
                )[0]
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "CCS_injection_vuln",
                "DESCRIPTION": messages(language, "vulnerable").format("CCS Injection"),
                "TIME": now(),
                "CATEGORY": "vuln",
                "SCAN_ID": scan_id,
                "SCAN_CMD": scan_cmd,
