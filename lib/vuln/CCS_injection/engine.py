...
def __CCS_injection(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if CCS_injection(target, port, timeout_sec, log_in_file, language, time_sleep,
             thread_tmp_filename, socks_proxy, scan_id, scan_cmd):
        info(messages(language, 140).format(target, port, 'CCS Injection'))
        __log_into_file(thread_tmp_filename, 'w', '0', language)
        data = json.dumps({'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'CCS_injection_vuln',
                           'DESCRIPTION': messages(language, 139).format('CCS Injection'), 'TIME': now(),
                           'CATEGORY': "vuln",
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep, language,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        for port in ports:
            ...
            if verbose_level > 3:
                info(
                    messages(language, 72).format(trying, total_req, num, total, target, port, 'CCS_injection_vuln'))
            while 1:
                try:
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, 141).format('CCS injection'))
            data = json.dumps({'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'CCS_injection_vuln',
                               'DESCRIPTION': messages(language, 141).format('CCS injection'), 'TIME': now(),
                               'CATEGORY': "scan", 'SCAN_ID': scan_id, 'SCAN_CMD': scan_cmd})
            __log_into_file(log_in_file, 'a', data, language)
        os.remove(thread_tmp_filename)

    else:
        warn(messages(language, 69).format('CCS_injection_vuln', target))
