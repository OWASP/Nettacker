...
def __CCS_injection(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if CCS_injection(target, port, timeout_sec, log_in_file, language, time_sleep,
        ...
        info(messages(language, "target_vulnerable").format(
            target, port, 'CCS Injection'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'CCS_injection_vuln',
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep, language,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format('CCS injection'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'CCS_injection_vuln',
