...
def extra_requirements_dict():
    return {"struts_vuln_ports": [80, 443]}
    ...
    try:
        ...
        if not s:
            ...
            if target_type(target) != "HTTP" and port == 80:
                target = "http://" + target
            random_string = "".join(
                random.choice("abcdefghijklmnopqrstuvwxyz") for i in range(7)
            )
            payload = (
            ...
            if (random_string in list(resp.headers.keys())) and (
                resp.headers[random_string] == random_string
            ):
                return True
            else:
                return False
