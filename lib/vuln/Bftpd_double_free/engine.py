...
def __Double_free(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if Double_free(target, port, timeout_sec, log_in_file, language, time_sleep,
        ...
        info(messages(language, "target_vulnerable").format(target, port,
                                                            'Double free vulnerability in bftpd before 1.8 allows remote authenticated users to cause a denial of service (daemon crash) via a (1) get or (2) mget command.	 CVE-2007-2010'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'Bftpd_double_free_vuln',
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format(
                'Bftpd_double_free	 CVE-2007-2010'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'Bftpd_double_free_vuln',
                               'DESCRIPTION': messages(language, "no_vulnerability_found").format('Bftpd_double_free       CVE-2007-2010'), 'TIME': now(),
                               'CATEGORY': "scan", 'SCAN_ID': scan_id, 'SCAN_CMD': scan_cmd}, language, thread_tmp_filename)
        os.remove(thread_tmp_filename)

    else:
        warn(messages(language, "input_target_error").format(
            'Bftpd_double_free_vuln', target))
