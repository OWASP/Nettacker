...
def extra_requirements_dict():
    return {"clickjacking_vuln_ports": [80, 443]}
...
def start(
    ...
    if (
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "clickjacking_vuln"
                    )
                )
            while 1:
                try:
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format("ClickJacking"))
            data = json.dumps(
                {
                    "HOST": target,
                    "USERNAME": "",
        ...
        warn(
            messages(language, "input_target_error").format("clickjacking_vuln", target)
        )
