...
def extra_requirements_dict():
    return {"Proftpd_vuln_ports": [21, 990]}
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "Proftpd_bypass_sqli_protection_vuln",
...
def start(
    ...
    if (
        ...
        for port in ports:
            port = int(port)
            t = threading.Thread(
                target=__bypass_sqli_protection,
                args=(
                    target,
                    int(port),
                    timeout_sec,
