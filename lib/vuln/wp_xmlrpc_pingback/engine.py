...
def extra_requirements_dict():
    return {"xmlrpc_pingback_vuln_ports": [80, 443]}
    ...
    if xmlrpc_pingback(
        ...
        info(
            messages(language, "target_vulnerable").format(
                target, port, "Wordpress XMLRPC pingback Vulnerability"
            )
        )
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "Wordpress_xmlrpc_pingback_vuln",
...
def start(
    ...
    if (
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format("xmlrpc_pingback"))
            data = json.dumps(
                {
                    "HOST": target,
                    "USERNAME": "",
