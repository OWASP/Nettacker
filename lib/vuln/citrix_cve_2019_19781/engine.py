...
def extra_requirements_dict():
    return {"citrix_cve_2019_19781_vuln_ports": [443]}
    ...
    try:
        ...
        if not s:
            ...
            if req0.status_code == 200 and "citrix" in req0.text.lower():
                ...
                req = requests.get(
                    req_url, timeout=10, headers=user_agent, verify=False
                )
    ...
    if citrix_vuln(
        ...
        info(
            messages(language, "target_vulnerable").format(
                target, port, "Citrix CVE-2019-19781 Vulnerability"
            )
        )
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "citrix_cve_2019_19781_vuln",
