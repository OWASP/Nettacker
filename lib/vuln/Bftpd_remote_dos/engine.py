...
def __remote_dos(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if remote_dos(target, port, timeout_sec, log_in_file, language, time_sleep,
        ...
        info(messages(language, "target_vulnerable").format(target, port,
                                                            'Bftpd is prone to an unspecified remote denial-of-service vulnerability.  CVE-2009-4593'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'Bftpd_remote_dos_vuln',
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format(
                'Bftpd_remote_dos CVE-2009-4593'), log_in_file, "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'Bftpd_remote_dos_vuln',
                               'DESCRIPTION': messages(language, "no_vulnerability_found").format('Bftpd_remote_dos  CVE-2009-4593'), 'TIME': now(),
                               'CATEGORY': "scan", 'SCAN_ID': scan_id, 'SCAN_CMD': scan_cmd}, language, thread_tmp_filename)
        os.remove(thread_tmp_filename)

    else:
        warn(messages(language, "input_target_error").format(
            'Bftpd_remote_dos_vuln', target))
