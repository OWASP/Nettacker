...
def csrf_vuln(target, port, timeout_sec, log_in_file, language, time_sleep,
                 thread_tmp_filename, socks_proxy, scan_id, scan_cmd, extra_requirements):
    try:
        ...
        if not s:
            ...
            try:
                ...
                options_enabled = requests.options(target, verify=False, headers=HEADERS, timeout=10)
                if "authorization" in options_enabled.headers["Access-Control-Allow-Headers"].lower():
                    return
            except Exception:
                pass
            ...
            try:
                ...
                is_present = set_cookie_session.get(target, headers=HEADERS, verify=False)
                for set_cookie in is_present.headers:
                    if set_cookie.lower() == "set-cookie":
                        if not ["strict", "lax"] in is_present.headers[set_cookie].lower():
                            global csrf_vulnerable
                            csrf_vulnerable = "CSRF due to missing SameSite Cookie attributes"
                            COUNT.add(True)
            except Exception:
                pass
            ...
            if forms:
                ...
                for form in forms:
                    ...
                    if form_details["action"].startswith("http"):
                        ...
                        final_target = scheme + "://" + domain + path + form_details["action"]
                    else:
                            ...
                            if key == token_name:
                                ...
                                try:
                                    if form_details["method"] == "post":
                                        token_abs = requests.post(final_target, headers=temp, data=data, timeout=10)
                                    elif form_details["method"] == "get":
                                        token_abs = requests.get(final_target, headers=temp, data=data, timeout=10)
                                except Exception:
                                    logging.exception("message")
                                if token_abs.status_code in [301, 302, 307, 200, 201, 204]:
                    ...
                    try:
                        if form_details["method"] == "post":
                            res = requests.post(final_target, headers=temp, data=data, timeout=10)
                        elif form_details["method"] == "get":
                            res = requests.get(final_target, headers=temp, data=data, timeout=10)
                    except Exception:
                        logging.exception("message")
...
def __csrf(target, port, timeout_sec, log_in_file, language, time_sleep,
                   thread_tmp_filename, socks_proxy, scan_id, scan_cmd, extra_requirements):
    if csrf_vuln(target, port, timeout_sec, log_in_file, language, time_sleep,
                    thread_tmp_filename, socks_proxy, scan_id, scan_cmd, extra_requirements):
        info(messages(language, "target_vulnerable").format(target, port, "Cross-Site Request Forgery"))
        __log_into_file(thread_tmp_filename, 'w', '0', language)
        data = json.dumps({'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'csrf_vuln',
                           'DESCRIPTION': messages(language, "vulnerable").format(csrf_vulnerable), 'TIME': now(),
                           'CATEGORY': "vuln",
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep, language,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        for port in ports:
            port = int(port)
            t = threading.Thread(target=__csrf,
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(trying, total_req, num, total, target, port, 'csrf_vuln'))
            while 1:
                try:
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format(
                'csrf'))
            data = json.dumps({'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'csrf_vuln',
                               'DESCRIPTION': messages(language, "no_vulnerability_found").format('csrf'), 'TIME': now(),
