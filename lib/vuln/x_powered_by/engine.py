    ...
    if powered_by(
        ...
        info(
            messages(language, "target_vulnerable").format(
                target, port, "X-powered-by disclosure in headers - " + header_server
            )
        )
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "x_powered_by_vuln",
                "DESCRIPTION": header_server[:128],
                "TIME": now(),
                "CATEGORY": "vuln",
                "SCAN_ID": scan_id,
                "SCAN_CMD": scan_cmd,
...
def start(
    ...
    if (
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "x_powered_by_vuln"
                    )
                )
            while 1:
                try:
        ...
        warn(
            messages(language, "input_target_error").format("x_powered_by_vuln", target)
        )
