...
def extra_requirements_dict():
    return {"vbulletin_0day_vuln_ports": [443, 80]}
    ...
    try:
        ...
        if not s:
            ...
            if (
                ...
                and port in extra_requirements_dict()["vbulletin_0day_vuln_ports"]
            ):
                target = "https://" + target
            user_agent = [
    ...
    if vbulletin_vuln(
        ...
        info(
            messages(language, "target_vulnerable").format(
                target, port, "vBulletin RCE 0day Vulnerability"
            )
        )
...
def start(
    ...
    if (
        ...
        if ports is None:
            ports = extra_requirements["vbulletin_0day_vuln_ports"]
