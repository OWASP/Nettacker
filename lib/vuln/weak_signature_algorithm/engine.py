...
def extra_requirements_dict():
    return {"weak_encryption_vuln_ports": [21, 25, 110, 143, 443, 587, 990, 1080, 8080]}
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "weak_encryption_algorithm_vuln",
                "DESCRIPTION": messages(language, "vulnerable").format(
                    "Weak Encryption Algorithm : sha1WithRSAEncryption"
                ),
                "TIME": now(),
                "CATEGORY": "vuln",
                "SCAN_ID": scan_id,
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        ...
                        target,
                        port,
                        "weak_encryption_algorithm_vuln",
                    )
                )
            while 1:
                try:
