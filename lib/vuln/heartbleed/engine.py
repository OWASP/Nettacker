...
def extra_requirements_dict():
    return {"heartbleed_vuln_ports": [21, 25, 110, 143, 443, 587, 990, 1080, 8080]}
...
def __heartbleed(
    ...
    if bleed(
        ...
        info(messages(language, "target_vulnerable").format(target, port, "heartbleed"))
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "heartbleed_vuln",
                "DESCRIPTION": messages(language, "vulnerable").format("heartbleed"),
                "TIME": now(),
                "CATEGORY": "vuln",
                "SCAN_ID": scan_id,
                "SCAN_CMD": scan_cmd,
...
def start(
    ...
    if (
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "heartbleed_vuln"
                    )
                )
            while 1:
                try:
        ...
        warn(messages(language, "input_target_error").format("heartbleed_vuln", target))
