...
def extra_requirements_dict():
    return {"host_header_vuln_ports": [80, 443]}
...
def host_header_vuln(
    ...
    try:
        ...
        if not s:
            ...
            host_headers = {
                "Host": "evil.com",
                "User-Agent": random.choice(useragents.useragents()),
            }
            try:
                req_host = requests.get(
                    target,
                    verify=False,
                    timeout=timeout_sec,
            ...
            try:
                req_forwarded_for = requests.get(
                    target,
                    verify=False,
                    timeout=timeout_sec,
                    headers=x_forwarded_for_headers,
                    allow_redirects=False,
                )
            ...
            try:
                req_forwarded_host = requests.get(
                    target,
                    verify=False,
                    timeout=timeout_sec,
                    headers=x_forwarded_host_headers,
                    allow_redirects=False,
                )
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "host_header_vuln"
                    )
                )
            while 1:
                try:
