...
def extra_requirements_dict():
    return {"http_cors_vuln_ports": [80, 443]}
...
def start(
    ...
    if (
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "http_cors_vuln"
                    )
                )
            while 1:
                try:
        ...
        warn(messages(language, "input_target_error").format("http_cors_vuln", target))
