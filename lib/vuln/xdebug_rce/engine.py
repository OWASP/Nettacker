...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep, language,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if ports is None:
            ports = extra_requirements["xdebug_vuln_ports"]
        ...
        for port in ports:
            port = int(port)
            t = threading.Thread(target=__xdebug,
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(trying, total_req, num, total, target, port, 'xdebug_vuln'))
            while 1:
                try:
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format(
                'xdebug'))
            data = json.dumps({'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'xdebug_vuln',
                               'DESCRIPTION': messages(language, "no_vulnerability_found").format('xdebug'), 'TIME': now(),
