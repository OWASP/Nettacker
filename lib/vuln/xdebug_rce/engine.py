...
def extra_requirements_dict():
    return {"xdebug_vuln_ports": [80, 443]}
...
def start(
    ...
    if (
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "xdebug_vuln"
                    )
                )
            while 1:
                try:
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "no_vulnerability_found").format("xdebug"))
            data = json.dumps(
                {
                    "HOST": target,
                    "USERNAME": "",
        ...
        warn(messages(language, "input_target_error").format("xdebug_vuln", target))
