    ...
    try:
        if socks_proxy is not None:
            ...
            if "@" in socks_proxy:
                ...
                socket.getaddrinfo = getaddrinfo
        r = requests.get(
            target + payload, timeout=timeout_sec, headers=user_agent
        ).content
        return True
    except:
        return False
...
def start(
    ...
    if (
        ...
        if (
            extra_requirements["wordpress_dos_cve_2018_6389_vuln_random_agent"][0]
            != "True"
        ):
            random_agent_flag = False
        if (
            extra_requirements["wordpress_dos_cve_2018_6389_vuln_no_limit"][0]
            != "False"
        ):
            limit = -1
        threads = []
        ...
        if target_type(target) == "SINGLE_IPv4" or target_type(target) == "DOMAIN":
            ...
            host = target_to_host(target)
            path = "/".join(
                target.replace("http://", "").replace("https://", "").rsplit("/")[1:]
            )
            url = http + "://" + host + "/" + path
        if (
            test(
                ...
                data = json.dumps(
                    {
                        ...
                        "DESCRIPTION": messages(
                            language, "no_vulnerability_found"
                        ).format("wordpress_dos_cve_2018_6389_vuln"),
                        "TIME": now(),
                        "CATEGORY": "scan",
                        "SCAN_ID": scan_id,
                        "SCAN_CMD": scan_cmd,
