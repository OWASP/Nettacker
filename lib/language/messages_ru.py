    ...
    return \
		{
			...
			"35": "введите методы ввода, например: «ftp_brute_users = test, admin & ftp_brute_passwds = read_from_file: /tmp/pass.txt&ftp_brute_port=21\"",
			...
			"49": "пропустить дублируемую цель (некоторые поддомены / домены могут иметь одинаковые IP-адреса и диапазоны)",
			...
			"68": "ftp-подключение к {0}: {1} не удалось, пропустив весь шаг [процесс {2} из {3}]! переход к следующему шагу",
			...
			"74": "smtp-соединение с {0}: {1} не удалось, пропустив весь шаг [процесс {2} из {3}]! переход к следующему шагу",
			"75": "входная цель для модуля {0} должна быть HTTP, пропуская {1}",
			"76": "ssh для {0}:{1} таймаут, пропуская {2}:{3}",
			"77": "ssh-соединение с {0}: {1} не удалось, пропустив весь шаг [процесс {2} из {3}]! переход к следующему шагу",
			"78": "ssh-соединение с {0}: {1} не удалось, пропустив весь шаг [процесс {2} из {3}]! переход к следующему шагу",
			"79": "открытый порт",
			...
			"85": "эта функция пока недоступна! запустите \"git clone https://github.com/viraintel/OWASP-Nettacker.git\" или \"pip install -U OWASP-Nettacker\" чтобы получить последнюю версию.",
			"86": "постройте график всех действий и информации, вы должны использовать вывод html. доступные графики: {0}",
			"87": "для использования функции графика ваше имя выходного файла должно заканчиваться символом \".html\" или \".htm\"!",
			"88": "строительный график ...",
			"89": "закончите строить график!",
			"90": "Графики тестирования проникновения",
			"91": "Этот график создан OWASP Nettacker. График содержит все действия модулей, карту сети и конфиденциальную информацию. Пожалуйста, не сообщайте этот файл никому, если он не является надежным",
			...
			"100": "пропускающая цель отверстия {0} и метод сканирования {1} --ping-before-scan истинны, и он не ответил!",
