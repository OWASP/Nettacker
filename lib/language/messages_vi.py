    ...
    return \
		{
			...
			"29": "thiết lập thời gian chờ đến {0} giây, nó là quá lớn, phải không? bằng cách chúng tôi đang tiếp tục ...",
			...
			"35": "nhập các phương thức đầu vào, ví dụ: \"ftp_brute_users = test, admin & ftp_brute_passwds = read_from_file: /tmp/pass.txt&ftp_brute_port=21\"",
			...
			"68": "kết nối ftp đến {0}: {1} không thành công, bỏ qua toàn bộ bước [process {2} of {3}]! đi tới bước tiếp theo",
			...
			"74": "kết nối smtp đến {0}: {1} không thành công, bỏ qua toàn bộ bước [process {2} of {3}]! đi tới bước tiếp theo",
			"75": "mục tiêu đầu vào cho {0} mô-đun phải là HTTP, bỏ qua {1}",
			"76": "kết nối ssh đến {0}: {1} timeout, bỏ qua {2}: {3}",
			"77": "kết nối ssh tới {0}: {1} không thành công, bỏ qua toàn bộ bước [process {2} of {3}]! đi tới bước tiếp theo",
			...
			"85": "tính năng này chưa có sẵn! hãy chạy \"git clone https://github.com/viraintel/OWASP-Nettacker.git\" hoặc \"pip install -U OWASP-Nettacker\" để có được phiên bản mới nhất.",
			"86": "xây dựng một đồ thị của tất cả các hoạt động và thông tin, bạn phải sử dụng đầu ra HTML. biểu đồ khả dụng: {0}",
			...
			"91": "Đồ thị này được tạo bởi OWASP Nettacker. Biểu đồ chứa tất cả các hoạt động mô-đun, bản đồ mạng và thông tin nhạy cảm, Vui lòng không chia sẻ tệp này với bất kỳ ai nếu nó không đáng tin cậy.",
			...
			"100": "bỏ qua toàn bộ mục tiêu {0} và phương pháp quét {1} vì -ping-trước-scan là đúng và nó đã không đáp ứng!",
