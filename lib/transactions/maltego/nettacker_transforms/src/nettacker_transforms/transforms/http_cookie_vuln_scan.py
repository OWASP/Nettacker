...
from lib.vuln.cookie.engine import start
...
@EnableDebugWindow
class HTTP_CookieVulnScan(Transform):
    ...
    def do_transform(self, request, response, config):
        ...
        for result in results:
            ...
            response += URL(url=url, title=result["DESCRIPTION"],
                            short_title="Set-Cookie header Vulnerability Found!",
                            link_label='http_cookie_vuln')
        return response
