...
from lib.brute.wp_xmlrpc.engine import start
...
@EnableDebugWindow
class WordpressXMLRPCBrute(Transform):
    ...
    def do_transform(self, request, response, config):
        ...
        scan_request.ports = (
            scan_request.ports.split(", ") if scan_request.ports is not None else None
        )
        scan_request.usernames = (
            scan_request.usernames.split(", ")
            if scan_request.usernames is not None
            else None
        )
        scan_request.passwords = (
            scan_request.passwords.split(", ")
            if scan_request.passwords is not None
            else None
        )
        start(
            scan_request.host,
            ...
            scan_request.passwords,
            scan_request.ports,
            scan_request.timeout_sec,
            scan_request.thread_no,
            1,
            1,
            "abcd",
            0,
            ...
            scan_request.socks_proxy,
            scan_request.retries,
            [],
            scan_id,
            "Through Maltego",
        )
        ...
        for result in results:
            response += NettackerBrute(
                ...
                link_label="Wordpress XMLRPC Brute found "
                + result["USERNAME"]
                + ":"
                + result["PASSWORD"],
            )
