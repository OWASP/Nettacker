...
from lib.vuln.host_header.engine import start
...
@EnableDebugWindow
class HostHeaderVulnScan(Transform):
    ...
    def do_transform(self, request, response, config):
        ...
        for result in results:
            ...
            response += URL(url=url, title=result["DESCRIPTION"],
                            short_title="Host/x-forwarded-for/x-forwarded-host header Vulnerability Found!",
                            link_label='host_header_vuln')
        return response
