...
from lib.scan.sender_policy.engine import start
...
@EnableDebugWindow
class JoomlaSenderPolicyScan(Transform):
    ...
    def do_transform(self, request, response, config):
        ...
        for result in results:
            ...
            sender_policy = result["DESCRIPTION"][
                result["DESCRIPTION"].find(":") + 1, -1
            ]
            response += URL(
                url=url,
                title=result["DESCRIPTION"],
                short_title=sender_policy + " Found!",
