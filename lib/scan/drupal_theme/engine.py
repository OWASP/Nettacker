...
def extra_requirements_dict():
    return {"drupal_theme_ports": [80, 443]}
...
def __drupal_theme(
    ...
    if theme:
        info(messages(language, "found").format(target, "drupal theme", theme))
...
def start(
    ...
    if (
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "drupal_theme_scan"
                    )
                )
            while 1:
                try:
        ...
        warn(
            messages(language, "input_target_error").format("drupal_theme_scan", target)
        )
