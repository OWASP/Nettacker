...
def __joomla_version(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if joomla_version(target, port, timeout_sec, log_in_file, language, time_sleep,
                      thread_tmp_filename, socks_proxy, scan_id, scan_cmd):
        info(messages(language, "found").format(target, "Sender Policy Framework Record : "+ result[0], " " + result[1]), log_in_file,
             "a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'joomla_version_scan',
                   'DESCRIPTION': messages(language, "found").format(target, "Sender Policy Framework Record : "+ result[0], " " + result[1]),
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep, language,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "not_found"), log_in_file,"a", {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'joomla_version_scan',
