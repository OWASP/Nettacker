...
def __wp_user_enum(target, port, timeout_sec, log_in_file, language, time_sleep,
    ...
    if wp_user_enum(target, port, timeout_sec, log_in_file, language, time_sleep,
        ...
        info(messages(language, "found").format(
            target, "Wordpress users found ", wp_users), log_in_file, "a",
             {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': port, 'TYPE': 'wp_user_enum_scan',
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if thread_write is 1 and verbose_level is not 0:
            info(messages(language, "not_found"), log_in_file, "a",
                 {'HOST': target, 'USERNAME': '', 'PASSWORD': '', 'PORT': '', 'TYPE': 'wp_user_enum_scan',
