    ...
    try:
        if s != "SA":
            try:
                ...
                s = sr1(
                    IP(dst=ip) / TCP(dport=closed_port, flags="S"), timeout=2, verbose=0
                )
                if s == None:
                    return None
                else:
                    ...
                    json.dumps(
                        {
                            ...
                            "PORT": port,
                            "TYPE": "port_scan",
                            "DESCRIPTION": messages(language, "port/type").format(
...
def __udp(
    ...
    try:
        ...
        try:
            ...
            service_name = ""
        info(
            messages(language, "port_found").format(
                host, str(port) + service_name, "UDP"
            ),
            log_in_file,
            "a",
            {
        ...
        try:
            if filter_port(host, port):
                info(
                    messages(language, "port_found").format(
                        host, str(port) + service_name, "TCP_CONNECT"
                    )
                )
                data = (
                    json.dumps(
...
def start(
    ...
    if (
        ...
        for port in ports:
            t = threading.Thread(
                target=stealth if stealth_flag else (__udp if udp_flag else connect),
                args=(
                    target,
                    int(port),
                    timeout_sec,
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "port_scan"
                    )
                )
            while 1:
                try:
        ...
        kill_switch = 0
        kill_time = (
            int(timeout_sec / 0.1) * (5 + timeout_sec) * 10
            if int(timeout_sec / 0.1) is not 0
            else 2
        )
        while 1:
            time.sleep(0.1)
