    ...
    try:
        ...
        if not s:
            ...
            try:
                ...
                user_agent = {"User-agent": random.choice(user_agent_list)}
                req_wordpress = requests.get(
                    req_wordpress_url, timeout=10, headers=user_agent
                )
                user_agent = {"User-agent": random.choice(user_agent_list)}
                req_joomla = requests.get(
                    req_joomla_url, timeout=10, headers=user_agent
                )
                user_agent = {"User-agent": random.choice(user_agent_list)}
                req_drupal = requests.get(
                    req_drupal_url, timeout=10, headers=user_agent
                )
            ...
            if req_wordpress.text != code_for_404 or req_wordpress.status_code == 403:
                ...
                req_drupal.status_code != code_for_404 or req_drupal.status_code == 403
            ):
                cms_name = "Drupal"
                return True
            elif (
                req_joomla.status_code != code_for_404 or req_joomla.status_code == 403
            ):
                cms_name = "Joomla"
                return True
            else:
