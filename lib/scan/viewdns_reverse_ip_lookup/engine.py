...
def start(
    ...
    if (
        ...
        while 1:
            try:
                res = requests.get(
                    "https://viewdns.info/reverseip/?host={0}&t=1".format(target),
                    timeout=timeout_sec,
                    headers=headers,
                    verify=True,
                ).text
        ...
        try:
            s = (
                "<table>"
                + res.rsplit("""<table border="1">""")[1].rsplit(
                    "<br></td></tr><tr></tr>"
                )[0]
            )
            table = ET.XML(s)
                ...
                json.dumps(
                    {
                        ...
                        "PORT": "",
                        "TYPE": "viewdns_reverse_ip_lookup_scan",
                        "DESCRIPTION": messages(language, "domain_found").format(
                            len(_values),
