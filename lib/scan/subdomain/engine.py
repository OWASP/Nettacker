    ...
    try:
        if socks_proxy is not None:
            ...
            if "@" in socks_proxy:
                ...
                socket.getaddrinfo = getaddrinfo
        req = requests.get(
            "https://certspotter.com/api/v0/certs?domain={0}".format(target),
            headers=headers,
        )
        subs = []
        ...
        while "<b>Next page</b></a>" not in results:
            ...
            if results.status_code is 200:
                for l in re.compile(
                    ...
                    if (
                        "*" not in target_to_host(l)
                        and target_to_host(l).endswith("." + target)
                        and target_to_host(l) not in subs
                    ):
...
def __comodo_crt(
    ...
    try:
        ...
        if results.status_code is 200:
            try:
                for list_of_domains in re.compile("<TD>(.*?)</TD>").findall(
                    results.text
                ):
        ...
        if results.status_code is 200:
            try:
                for l in re.compile(
                    ...
                    if (
                        "*" not in target_to_host(l)
                        and target_to_host(l.strip()).endswith("." + target)
                        and target_to_host(l.strip()) not in subs
                    ):
...
def __get_subs(
        ...
        if verbose_level > 3:
            info(
                messages(language, "trying_process").format(
                    trying, total_req, num, total, target, "(subdomain_scan - netcraft)"
                )
            )
        t = threading.Thread(
            target=__netcraft,
    ...
    try:
        kill_time = (
            -1
            if extra_requirements["subdomain_scan_time_limit_seconds"][0] == -1
            else int(
                int(extra_requirements["subdomain_scan_time_limit_seconds"[0]]) / 0.1
            )
        )
    except:
        kill_time = -1
    while 1:
        ...
        kill_switch += 1
        try:
            if threading.activeCount() is 1 or (
                kill_time is not -1 and kill_switch is kill_time
            ):
                break
...
def start(
    ...
    if (
                ...
                json.dumps(
                    {
                        ...
                        "PORT": "",
                        "TYPE": "subdomain_scan",
                        "DESCRIPTION": messages(language, "subdomain_found").format(
        ...
        warn(messages(language, "input_target_error").format("subdomain_scan", target))
        return []
