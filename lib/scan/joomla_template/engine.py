...
def extra_requirements_dict():
    return {"joomla_template_ports": [80, 443]}
    ...
    try:
        ...
        if not s:
            ...
            try:
                ...
                web_template = "".join(
                    set(re.findall("/templates/(.+?)/", r.text, re.IGNORECASE))
                )
                admin_template = "".join(
                    set(
                        re.findall(
                            "/administrator/templates/(.+?)/", r2.text, re.IGNORECASE
                        )
                    )
                )
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": port,
                "TYPE": "joomla_template",
                "DESCRIPTION": messages(language, "found").format(
                    target,
                    "Web Template and Admin Template Found",
                    web_template + " " + admin_template,
                ),
                "TIME": now(),
                "CATEGORY": "scan",
                "SCAN_ID": scan_id,
...
def start(
    ...
    if (
        ...
        if ports is None:
            ports = extra_requirements["joomla_template_ports"]
            ...
            if verbose_level > 3:
                info(
                    messages(language, "trying_message").format(
                        trying, total_req, num, total, target, port, "joomla_template"
                    )
                )
            while 1:
                try:
        ...
        warn(messages(language, "input_target_error").format("joomla_template", target))
