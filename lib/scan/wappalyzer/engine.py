...
def _parse_webpage(
    ...
    try:
        ...
        webpage["scripts"] = [
            script["src"] for script in webpage["html"].findAll("script", src=True)
        ]
        webpage["metatags"] = {
            meta["name"].lower(): meta["content"]
            for meta in webpage["html"].findAll(
...
def analyze(
    ...
    obj = json.loads(
        pkg_resources.resource_string(__name__, "apps.json").decode()
        if version() is 3
        else pkg_resources.resource_string(__name__, "apps.json")
    )
    apps = obj["apps"]
    detected = []
        ...
        data = json.dumps(
            {
                ...
                "PASSWORD": "",
                "PORT": "",
                "TYPE": "wappalyzer_scan",
                "DESCRIPTION": x[0] + ": " + ", ".join(x[1]),
                "TIME": now(),
                "CATEGORY": "scan",
                "SCAN_ID": scan_id,
                "SCAN_CMD": scan_cmd,
