...
def extra_requirements_dict():
    return {"joomla_user_enum_ports": [80, 443]}
    ...
    try:
        ...
        if not s:
            ...
            joomla_users = list(
                set(
                    re.findall(
                        "<author>(.+?) \((.+?)\)</author>", r.text, re.IGNORECASE
                    )
                )
            )
            temp_var = []
...
def start(
    ...
    if (
        ...
        if ports is None:
            ports = extra_requirements["joomla_user_enum_ports"]
