...
def login(
    ...
    while 1:
        try:
            ...
            if exit is retries:
                warn(
                    messages(language, "ssh_connection_timeout").format(
    ...
    if flag is 0:
        try:
            ...
            if timeout_sec is not None:
                ...
                ssh.connect(
                    hostname=target, username=user, password=passwd, port=int(port)
                )
            info(
                messages(language, "user_pass_found").format(user, passwd, target, port)
            )
...
def __connect_to_port(
    ...
    while 1:
        try:
            ...
            if timeout_sec is not None:
                ssh.connect(
                    target, username="", password="", timeout=timeout_sec, port=port
                )
            else:
            ...
            if "Authentication failed." in str(ssherr):
                ...
                if exit is retries:
                    error(
                        messages(language, "ssh_connection_failed").format(
