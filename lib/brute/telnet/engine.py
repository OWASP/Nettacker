...
def login(user, passwd, target, port, timeout_sec, log_in_file, language, retries, time_sleep, thread_tmp_filename,
    ...
    while 1:
        try:
            ...
            exit += 1
            if exit is retries:
                warn(messages(language, 182).format(target, port, user, passwd))
                return 1
...
def __connect_to_port(port, timeout_sec, target, retries, language, num, total, time_sleep, ports_tmp_filename,
    ...
    while 1:
        try:
            ...
            exit += 1
            if exit is retries:
                error(messages(language, 183).format(target, port, str(num), str(total)))
                try:
...
def test_ports(ports, timeout_sec, target, retries, language, num, total, time_sleep, ports_tmp_filename,
    ...
    for port in _ports:
        ...
        trying += 1
        if verbose_level > 3:
            info(messages(language, 72).format(trying, total_req, num, total, target, port, 'telnet_brute'))
        while 1:
            n = 0
...
def start(target, users, passwds, ports, timeout_sec, thread_number, num, total, log_in_file, time_sleep,
    ...
    if target_type(target) != 'SINGLE_IPv4' or target_type(target) != 'DOMAIN' or target_type(target) != 'HTTP':
        ...
        if passwds is None:
            passwds = extra_requirements["telnet_brute_passwds"]
        if ports is None:
            ports = extra_requirements["telnet_brute_ports"]
        ...
        warn(messages(language, 69).format('telnet_brute', target))
