...
def start(shellcode):
    ...
    while t:
        ...
        if version() is 3:
            eax_1 = (
                binascii.b2a_hex(
                    ("".join(random.choice(chars) for i in range(1))).encode("latin-1")
                )
            ).decode("latin-1")
        ...
        if "push" in line and "$0x" in line and "," not in line and len(line) > 14:
            ...
            while t:
                ...
                if version() is 3:
                    ebx_1 = (
                        binascii.b2a_hex(
                            ("".join(random.choice(chars) for i in range(4))).encode(
                                "latin-1"
                            )
                        )
                    ).decode("latin-1")
                ...
                if (
                    str("00") not in str(ebx_1)
                    and str("00") not in str(ebx_2)
                    and "-" in ebx_2
                    and len(ebx_2) >= 7
                    and len(ebx_1) >= 7
                    and "-" not in ebx_1
